  ...
  return (
    <div className="mt-5 min-w-[300px] lg:min-w-[500px]">
      {isLoading ? (
        ...
        blogs.map((blog) => {
          return (
            <div className="flex lg:space-x-5 justify-between items-center border-b border-slate-200 dark:border-slate-700">
              <DraftBlogCard key={blog.id} blog={blog} />
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button classname="bg-red-500 dark:bg-red-500">Delete</Button>
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>
        ...
        <p className="text-center">You have no drafts.</p>
      )}
    </div>
  );
}
